...
document.addEventListener("submit", (e) => {
  ...
  if (ratingValue) {
    for (let i = 0; i < ratingValue; i++) {
      ...
      span.classList.add("review-star");
      span.textContent = "★";
      stars.push(span);
    }
  }
  ...
  const totalStars = stars.map(() => "★").join("");
  // totalStars.classList.add('review-star')
  newParagraph.textContent = `${totalStars} ${text}`;
  textDisp.appendChild(newParagraph);
  textArea.value = "";
});
